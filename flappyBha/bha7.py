...
def bird_animation():
    new_bird = birds_frame[bird_index]
...
bird_midflap = pygame.image.load('sprites/redbird-midflap.png').convert_alpha()
bird_upflap = pygame.image.load('sprites/yellowbird-upflap.png').convert_alpha()
birds_frame = [bird_downflap, bird_midflap, bird_upflap]
bird_index = 0
bird_surface = birds_frame[bird_index]
bird_rect = bird_surface.get_rect(center = (50,512/2))

BIRDFLAP = pygame.USEREVENT + 1 # new user event + 2 for other
pygame.time.set_timer(BIRDFLAP,2000) #200 millisecond
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == BIRDFLAP:
            ...
            brid_surface,bird_rect = bird_animation()
